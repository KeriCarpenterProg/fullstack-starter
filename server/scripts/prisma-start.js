  ...
  try {
    ...
    for (const id of existingMigrations) {
      try {
        ...
        console.warn(`[Baseline] Could not mark ${id}: ${err.message}. Continuing.`);
      }
    }
    try {
      run('npx prisma migrate deploy');
      ...
      console.warn('[Baseline] migrate deploy still failing; falling back to prisma db push.');
      run('npx prisma db push');
      console.log('[Fallback] prisma db push completed. NOTE: migration history is synthetic; create a proper baseline later.');
    }
  }
}
