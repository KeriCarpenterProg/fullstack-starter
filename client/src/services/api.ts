...
export const projectsAPI = {
  ...
  async createProject(title: string, description?: string, category?: string): Promise<Project> {
    const payload: Record<string, any> = { title };
    if (description && description.trim().length) payload.description = description;
    if (category && category.trim().length) payload.category = category;
    const response = await api.post('/projects', payload);
    return response.data;
  },

  async updateProject(id: string, title?: string, description?: string, category?: string): Promise<Project> {
